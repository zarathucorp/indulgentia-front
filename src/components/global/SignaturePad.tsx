...
const SignaturePad = () => {
  ...
  const [initialSignatureUrl, setInitialSignatureUrl] = useState<string | null>(
    null
  );
  const [isFileSelected, setIsFileSelected] = useState<boolean>(false);
  const { toast } = useToast();
  const { data: signatureData } = useSWRImmutable(
    process.env.NEXT_PUBLIC_API_URL + "/user/settings/signature",
    async (url: string) => {
  ...
  async function onSubmit(data: CreateSignatureFormValues) {
    ...
    try {
      const result = await axios.post(
        process.env.NEXT_PUBLIC_API_URL + "/user/settings/signature/file",
        sendData,
        {
          timeout: 120000,
          headers: {
  ...
  return (
    <div className="flex flex-col items-center px-4">
      <div
        id="onborda-step3"
        className="relative border-2 border-gray-500 w-full max-w-[500px] h-[200px] signature-container"
      >
        {!isSigned && (
          <div
      ...
      <div className="relative max-w-[500px] h-[200px] w-full my-4 mx-4 sm:mx-4 lg:mx-auto">
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            <div id="onborda-step6" className="flex justify-center">
              <SignatureFileField
                form={form}
              ...
              />
              {isFileSelected ? (
                <ActionButton type="submit" className="my-1.5">
                  {isSubmitting && <Spinner />}&nbsp;서명 파일 업로드
                </ActionButton>
              ) : (
                <></>
              )}
    ...
    <FormField
      ...
      render={({ field }) => (
        <FormItem className="relative">
          ...
          <FormDescription className="absolute top-11 left-0 whitespace-nowrap overflow-visible w-max">
            서명 파일을 업로드할 수 있습니다.
          </FormDescription>
