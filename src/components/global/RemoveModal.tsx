  ...
  return (
    <>
      <DialogHeader>
        ...
        <DialogDescription>
          <p>
            이 작업은 {removeTypeDescription[removeType]} &quot;{targetEntity}
            &quot;를 영구적으로 삭제합니다. 이 작업은 되돌릴 수 없거나, 복구에
            많은 비용이 발생합니다.
          </p>
          <p>
      ...
      <div className="flex items-center space-x-2">
        <div className="grid flex-1 gap-2">
          <ConfirmationInput
            ...
            onChange={(e) => setConfirmEntityName(e.target.value)}
          />
...
export function LeaderTeamExitModal({
  isOpen,
  setIsOpen,
  teamInfo,
  teamMutate,
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            <p>
              팀을 삭제합니다. 팀과 팀에서 생성한 모든 데이터를 영구적으로
              삭제합니다. 이 작업은 되돌릴 수 없거나, 복구에 많은 비용이
              발생합니다.
            </p>
            <p>
          ...
          <Button
            ...
            onClick={async () => {
              try {
                ...
                await axios.delete(
                  `${process.env.NEXT_PUBLIC_API_URL}/user/team/${teamInfo?.id}`
                );
                toast({
                  title: "삭제 완료",
...
export function AccountRemoveModal({
  isOpen,
  setIsOpen,
  userInfo,
  userMutate,
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            <p>
              계정을 삭제합니다. 계정과 계정에서 생성한 모든 데이터를 영구적으로
              삭제합니다. 이 작업은 되돌릴 수 없거나, 복구에 많은 비용이
              발생합니다.
            </p>
            <p>
            ...
            onClick={async () => {
              try {
                ...
                const response = await axios.delete(
                  `${process.env.NEXT_PUBLIC_API_URL}/auth/remove`
                );
