...
export default function MyBucketList({
  ...
  useEffect(() => {
    const hasVistedDashboardBucket = localStorage.getItem(
      "hasVistedDashboardBucket"
    );
    console.log("hasVistedDashboardBucket", hasVistedDashboardBucket);
    if (!hasVistedDashboardBucket) {
      startOnborda("dashboard-bucket");
      localStorage.setItem("hasVistedDashboardBucket", "true");
    }
  }, [startOnborda]);
      ...
      <Card>
        <CardHeader>
          <CardTitle className="grid grid-cols-3 md:grid-cols-4 xl:grid-cols-6">
            ...
            {isLeader ? (
              <Link
                href={`/dashboard/project/${projectId}/setting`}
                className="col-start-2 md:col-start-3 xl:col-start-5"
              >
                <Button
                  id="onborda-step3"
                  variant={"outline"}
            ...
            <Link
              href={`/dashboard/bucket/create?project=${projectId}`}
              className="col-start-3 md:col-start-4 xl:col-start-6"
            >
              <Button id="onborda-step4" className="w-full ">
                새 버킷
              </Button>
            </Link>
          </CardTitle>
        ...
        <CardContent>
          ...
          <Table id="onborda-step5">
            <TableHeader>
              <TableRow>
                <TableHead id="onborda-step6">버킷 이름</TableHead>
                <TableHead id="onborda-step7" className="hidden md:table-cell">
                  상태
                </TableHead>
                <TableHead id="onborda-step8" className="hidden md:table-cell">
                  버킷 매니저
                </TableHead>
                <TableHead id="onborda-step9" className="hidden xl:table-cell">
                  GitHub 연동
                </TableHead>
                {/* <TableHead className="hidden xl:table-cell"></TableHead> */}
                {/* <TableHead className="hidden md:table-cell">수행 기간</TableHead> */}
                <TableHead id="onborda-step10">
              ...
              {bucketList.map((bucket: BucketType) => {
                return (
                  <TableRow
                    ...
                    onClick={() =>
                      toggleDownload && handleBucketClick(bucket.id)
                    }
                  >
                      ...
                      {toggleDownload &&
                        selectedBuckets.includes(bucket.id) && (
                          <Badge className="absolute top-0 left-0">
                            {selectedBuckets.indexOf(bucket.id) + 1}
                          </Badge>
                        )}
                      {bucket.title}
                    </TableCell>
                    ...
                    <TableCell className="hidden md:table-cell">
                      {!bucket.manager_first_name && !bucket.manager_last_name
                        ? "이름 미설정"
                        : (bucket.manager_last_name ?? "") +
                          (bucket.manager_first_name ?? "")}
                    </TableCell>
                    <TableCell className="hidden xl:table-cell">
                      {bucket.gitrepos === null ? "아니오" : "예"}
                    </TableCell>
                    <TableCell>
                      {toggleDownload ? (
        ...
        <CardFooter className="grid grid-cols-3 md:grid-cols-4 xl:grid-cols-6">
          ...
          <Button
            ...
            className="col-span-2 col-start-3 md:col-start-4 xl:col-start-6"
          >
            버킷 다운로드
          </Button>
        </CardFooter>
        ...
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-gray-900/75 text-gray-50">
          <div className="flex items-center justify-between">
            <div className="flex-1">
              선택된 노트: <strong>{selectedBuckets.length}</strong> 개
            </div>
            <div className="flex space-x-2">
              <Button onClick={handleSelectAll}>
                {selectedBuckets.length === bucketList?.length
                  ? "전체 선택 해제"
                  : "전체 선택"}
              </Button>
              <Dialog>
                <DialogTrigger asChild disabled={selectedBuckets.length === 0}>
                  <Button disabled={selectedBuckets.length === 0}>
                    다운로드
                  </Button>
                </DialogTrigger>
