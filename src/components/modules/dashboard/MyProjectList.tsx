...
export default function MyProjectList({
  ...
  useEffect(() => {
    const hasVistedDashboardProject = localStorage.getItem(
      "hasVistedDashboardProject"
    );
    console.log("hasVistedDashboardProject", hasVistedDashboardProject);
    if (!hasVistedDashboardProject) {
      startOnborda("dashboard-project");
      localStorage.setItem("hasVistedDashboardProject", "true");
    }
  }, [startOnborda]);
      ...
      <Card>
        <CardHeader>
          <CardTitle className="grid grid-cols-3 md:grid-cols-4 xl:grid-cols-6">
            ...
            >
              <Button id="onborda-step3" className="w-full righ">
                새 프로젝트
              </Button>
            </Link>
          </CardTitle>
        ...
        <CardContent>
          <Table id="onborda-step4">
            <TableHeader>
              <TableRow>
                <TableHead id="onborda-step5">프로젝트 이름</TableHead>
                <TableHead id="onborda-step6" className="hidden md:table-cell">
                  연구책임자
                </TableHead>
                <TableHead id="onborda-step7" className="hidden md:table-cell">
                  과제번호
                </TableHead>
                <TableHead id="onborda-step8" className="hidden xl:table-cell">
                  연구 시작일
                </TableHead>
            ...
            <TableBody>
              {projectList.map((project: ProjectType) => (
                <TableRow
                  ...
                  onClick={() =>
                    toggleDownload && handleProjectClick(project.id)
                  }
                >
        ...
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-gray-900/75 text-gray-50">
          <div className="flex items-center justify-between">
            <div className="flex-1">
              선택된 노트: <strong>{selectedProjects.length}</strong> 개
            </div>
