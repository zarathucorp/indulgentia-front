...
export function TeamForm() {
	...
	const onTransferButtonClick = async (newLeaderId: UUID) => {
		try {
			...
			toast({
				title: "팀장 권한 이전 실패",
				description: `팀장 권한이 이전되지 않았습니다. ${error?.response?.data?.detail ?? error.message}`,
			});
		}
	};
	...
	const onAcceptInvitation = async (invitationId: UUID, teamId: UUID) => {
		try {
			...
			toast({
				title: "초대 수락 실패",
				description: `초대를 수락하지 못했습니다. ${error?.response?.data?.detail ?? error.message}`,
			});
		}
	};
	...
	const onDeclineInvitation = async (invitationId: UUID, teamId: UUID) => {
		try {
			...
			toast({
				title: "초대 거절 실패",
				description: `초대를 거절하지 못했습니다. ${error?.response?.data?.detail ?? error.message}`,
			});
		}
	};
	...
	const onCancelInvite = async (invitationId: UUID) => {
		try {
			...
			toast({
				title: "초대 취소 실패",
				description: `초대를 취소하지 못했습니다. ${error?.response?.data?.detail ?? error.message}`,
			});
		} finally {
			invitationMutate();
			invitationSendListMutate();
