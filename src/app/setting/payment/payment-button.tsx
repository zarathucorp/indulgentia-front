...
function StartTeamPlanButton(
  props: React.ButtonHTMLAttributes<HTMLButtonElement>
) {
  const { teamInfo, hasTeam, error, isLoading, mutate, isLeader } =
    useTeamInfo();

  return (
    <Button id="onborda-step2" disabled={isLoading || !hasTeam} {...props}>
      {!isLoading && !hasTeam
        ? "팀 플랜을 시작하려면 먼저 팀을 만들어야 합니다."
        : "팀 플랜 시작하기"}
    </Button>
  );
}
