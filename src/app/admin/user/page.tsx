...
import {
  AdminUserTable,
  User,
} from "@/components/modules/admin/table/AdminUserTable";
...
export default function AdminUserPage() {
  const { data, isLoading, error, mutate } = useSWR<FetchResultType<User[]>>(
    process.env.NEXT_PUBLIC_API_URL + `/admin/user/list`,
    fetcher,
    {
  ...
  return (
    <ContentLayout title="Admin User">
      ...
      {error ? (
        ...
        data && <AdminUserTable data={data.users} mutate={mutate} />
      )}
    </ContentLayout>
  );
}
      ...
      <BreadcrumbList>
        ...
        {team_id && (
          <>
            <BreadcrumbItem>
              <BreadcrumbLink asChild>
                <Link href={`/admin/user?team_id=${team_id}`}>
                  ?team_id={team_id}
                </Link>
              </BreadcrumbLink>
            </BreadcrumbItem>
