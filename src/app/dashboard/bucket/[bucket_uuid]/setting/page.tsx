...
import {
  EditBucketForm,
  CreateBucketFormValues,
} from "@/components/modules/dashboard/bucket/BucketForm";
...
import {
  ...
  GithubRepoType,
} from "./ConnectedGithubRepository";
...
const fetcher = (url: string) =>
  axios.get(url).then((res) => res.data.data.github_token);
...
const InstallationRepoSelector: React.FC<{
  ...
  useEffect(() => {
    if (selectedInstallation && token) {
      ...
      axios
        .get(
          `/next-api/github/repos?token=${token}&installation_id=${selectedInstallation}`
        )
  ...
  const getGitHubUsernameById = (targetId: number) =>
    installations.find((installation) => installation.id === targetId)?.account
      .login || "";
...
const BucketSetting: React.FC = () => {
  ...
  } = useSWRImmutable<CreateBucketFormValues & { id: UUID }>(
    `${process.env.NEXT_PUBLIC_API_URL}/dashboard/bucket/${params.bucket_uuid}`,
    async (url: string) => {
  ...
  const { data: breadcrumbData, isLoading: isBreadcrumbLoading } = useSWR(
    `${process.env.NEXT_PUBLIC_API_URL}/dashboard/bucket/${params.bucket_uuid}/breadcrumb`,
    async (url: string) => axios.get(url).then((res) => res.data.data)
  );
  ...
  const {
    ...
    `${process.env.NEXT_PUBLIC_API_URL}/dashboard/bucket/${params.bucket_uuid}/github_repo`,
    async (url: string) => {
      const { data } = await axios.get(url);
      return data.data;
    }
  ...
  const handleConnectRepository = async () => {
    ...
    const hasMatchingRepo = (
      ...
      repos.some(
        (repo) =>
          repo.git_username.toLowerCase() === username.toLowerCase() &&
          repo.git_repository.toLowerCase() === repository.toLowerCase()
      );

    try {
      if (
        connectedGithubRepos &&
        hasMatchingRepo(connectedGithubRepos, gitUsername, gitRepository)
      ) {
      ...
      toast({
        title: "GitHub Repository 연동 실패",
        description: `GitHub Repository 연동에 실패하였습니다. ${
  ...
  return (
    <div>
      ...
      {isLoadingBucketInfo ? (
        ...
        bucketInfo && (
          <EditBucketForm bucketInfo={bucketInfo} mutate={mutateBucketInfo} />
        )
      )}
          ...
          {connectedGithubRepos.map((repo: GithubRepoType) => (
            <div className="bg-white rounded-lg shadow-md p-4" key={repo.id}>
              <div className="flex items-center justify-between">
                ...
                <RemoveRepositoryModal
                  repo={repo}
                  mutateConnectedGithubRepos={mutateConnectedGithubRepos}
                />
              </div>
            </div>
          ))}
