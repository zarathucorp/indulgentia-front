            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
              {filteredNotes?.map((note: NoteTypeWithUserSetting) => (
                <Card
                  ...
                  className={cn(`${selectedNotes.includes(note.id) ? "bg-blue-100" : ""}`, `${toggleDownload ? "cursor-pointer" : ""}`)}
                  ...
                  <CardContent>
                    <div className="grid w-full items-center gap-4">
                      <div className="flex flex-col space-y-1.5">
                        <Label className={`${toggleDownload ? "cursor-pointer" : ""}`}>생성일시: {convertKST(note.created_at)}</Label>
                      </div>
                      <div className="flex flex-col space-y-1.5">
                        <Label className={`${toggleDownload ? "cursor-pointer" : ""}`}>작성자: {note.user_setting.last_name} {note.user_setting.first_name}</Label>
                      </div>
                    </div>
                  </CardContent>
            ...
            <div className="flex space-x-2">
              ...
              <Dialog>
                <DialogTrigger asChild disabled={selectedNotes.length === 0}>
									<Button disabled={selectedNotes.length === 0}>다운로드</Button>
                </DialogTrigger>
